...
//                });
                ...
                sys_core.object.extend(sys_core, {request: function request(_parameter_settings) {
                        ...
                        ajax.request.setRequestHeader("contentType", "application/x-www-form-urlencoded; charset=utf-8");
                ...
                sys_core.object.extend(sys_core.component, {
                    ...
                    select: function (_object) {
                        ...
                        if (sys_core.isChrome()) {
                            ...
                            cs_input.attr('value', _object.textDefault);
                            ...
                            _object.data.load(function () {
                                ...
                                if (!_object.data.isEmpty())
                                    cs_input.attr('value', _object.textEmpty);
                                else
                            ...
                            event_reset = function () {
                                ...
                                if (this.isEmpty())
                                    ...
                                    cs_searchText.attr('value', _object.textEmpty);
//                                    cs_selectOptions.content('');
                                }
                            };
                ...
                sys_core.object.extend(sys_core, {
                    memory: function memory(_parameter_settings) {
                        ...
                        sys_core.object.extend(memory['proxy'], {
                            load_data: function (fn, exp, scope) {
                                var me = this;

                                me['progress-in'] = true;
                                me['progress-complete'] = false;

                                if (sys_core.isDefined(me['url'])) {
                                    if (me['url'] === "") {
                                ...
                                sys_core.request({
                                    url: me['url'] + '?get-data-model*' + sys_core.newID('data-model'),
                                    success: function () {
                                        var responseText = this.responseText;
                                        if (exp) {
                                            responseText = exp(responseText);
                                        }
                                        me.load_complete.call(me, responseText, fn, scope);
                                    },
                                    failure: function () {
                                        me.load_complete.call(me, '', fn, scope);
                                    }
                                });
                            },
                            load_complete: function (response, fn, scope) {
                                var me = this;
                        ...
                        sys_core.object.extend(memory, {
                            load: function (self_fn, exp) {
                                ...
                                if (Array.isArray(me['data'])) {
                                    if (me['data'].length > 0) {
                                        self_fn.call(me, me['data']);
                                        return true;
                                    }
                                }

                                me['proxy'].load_data(function (dt) {
                                    me.loadData(dt);
                                    self_fn.call(me, me['data']);
                                }, exp, me);
                            },
...
//                        _boxClose.down().css('width', '10px');
                ...
                sys_core.object.extend(sys_core, {
                    icon: function (config) {
                        var me = sys_core.JLib('html').create_element('img');

                        config = config || {};

                        me.attr('src', config.path || '');
                        me.attr('height', config.height || '');

                        me.mark_component(['icon', sys_core.newID('icon')]);
                        return me;
                    }
                });

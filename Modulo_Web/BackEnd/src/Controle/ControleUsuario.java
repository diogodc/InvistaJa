...
public class ControleUsuario {
	...
	public ModeloUsuario autenticar(@PathParam("jUsuario") String jUsuario){
		try{
			...
			return dAutenticacao.autenticarUsuario(mUsuario);
		}catch(Exception ex){
	...
	public ArrayList<ModeloUsuario> listar(@PathParam("sUsuarioLogado_Id") String sUsuarioLogado_Id){
		try{
			...
			if (sUsuarioLogado_Id.isEmpty() || sUsuarioLogado_Id.equals("0")){return null;}
			if (dAutenticacao.consultar(Integer.parseInt(sUsuarioLogado_Id)).size() == 0){return null;}
			return dAutenticacao.consultarUsuarios();
		}catch(Exception ex){
	...
	public ArrayList<ModeloUsuario> consultar(@PathParam("sUsuarioLogado_Id") String sUsuarioLogado_Id,
		...
		try{
			...
			return dAutenticacao.consultarUsuarios(Integer.parseInt(sUsuario_Id));
		}catch(Exception ex){
